...
class helpStepCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('kitpages:step:help-step')
            ->addArgument('stepName', InputArgument::OPTIONAL, 'step name')
            ->setHelp(<<<EOT
The <info>kitpages:step:help-step</info> display help
EOT
            )
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $stepConfigList = $this->getContainer()->getParameter("kitpages_step.shared_step_list");
        if (!$stepName) {
